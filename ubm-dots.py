...
REPO_OWNER = "deeerain"
REPO_NAME = "ubm-dots"
VERSION = "0.1.1-1"  # Update this when releasing new versions (sync with PKGBUILD)
...
class Config:
    ...
    def repo(self) -> RepoConfig:
        ...
        return RepoConfig(
            owner=repo.get("owner", REPO_OWNER), name=repo.get("name", REPO_NAME)
        )
            ...
            with urllib.request.urlopen(url, timeout=30) as response:
                ...
                with open(filename, "wb") as f:
                    with typer.progressbar(
                        length=total_size, label=f"Downloading {filename.name}"
                    ) as progress:
                        while True:
...
class UBM:
    ...
    def __init__(self, update_service: UpdateService, config: Config):
        ...
        self.modules: Iterator[Path] = (
            DOTS_FOLDER.iterdir() if DOTS_FOLDER.exists() else iter([])
        )
    ...
    def update(self, dry_run: bool = False):
        ...
        try:
            ...
            typer.echo(
                f"Successfully updated to version {self.update_service.latest_version}"
            )
    ...
    def restore(self) -> None:
        ...
        self.restart_services()
        typer.echo(
            f"Restored {restored_count} configurations, removed {removed_count} symlinks"
        )
            ...
            if home_module.is_symlink():
                try:
                    ...
                    if str(target) == str(module):
                        ...
                        typer.echo(
                            f"  ⚠ {module_name} -> linked to {target} (expected {module})"
                        )
                except:
...
def update(
    ...
    force: bool = typer.Option(
        False, "--force", help="Force update even if not needed"
    ),
):
    """
    Update UBM to the latest version.
    ...
    if dry_run:
        ...
        if ubm.check_updates():
            typer.echo(
                f"Would update from {VERSION} to {update_service.latest_version}"
            )
        else:
...
def install_command(
    force: bool = typer.Option(
        False, "--force", "-f", help="Force reinstall even if exists"
    ),
    debug: bool = typer.Option(False, "--debug", help="Enable debug mode"),
):
    """
...
if __name__ == "__main__":
    ...
    if config.update_check_on_startup and not config.auto_update:
        if ubm.check_updates():
            typer.echo(
                f"Update available: {VERSION} -> {update_service.latest_version}"
            )
